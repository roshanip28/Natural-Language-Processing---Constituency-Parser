...
class Tree(object):
    ...
    def markov(self):
        node_list=list(self.bottomup())
        for r_node in node_list:
            if len(r_node.children)==2 and r_node.parent:
                if "*" in r_node.label:
                    r_node.label = r_node.label + "@" + r_node.parent.label

    def de_markov(self):
        node_list = list(self.bottomup())
        for r_node in node_list:
                if "@" in r_node.label:
                    r_node.label = r_node.label.split("@")[0]
    ...
    def sibling(self):
        ...
        for r_node in node_list:
            if len(r_node.children) == 2 and r_node.parent:
                if "*" in r_node.label:
                    for c in r_node.parent.children:
                        if c.label!=r_node.label:
                            r_node.label=r_node.label + "&" + c.label
